...
st.set_page_config(
    page_title='Dashboard de Salário da Área de Dados',
    page_icon='📊',
    layout='wide'
)
...
# --- Criar barra lateral com filtros ---
...
st.markdown('Explore os dados salariais da área de dados nos utimos anos. Utilize os filtros à esquerda para refinar sua análise.')
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_cargo = px.bar(
            ...
            y='cargo',
            orientation='h',
            title='Top 10 cargo por salário médio',
            labels={'usd':'Média salárial anual (USD)', 'cargo':''}
        )
        grafico_cargo.update_layout(title_x=0.5, yaxis={'categoryorder':'total ascending'})
...
with col_graf3:
    if not df_filtrado.empty:
        ...
        st.warning('Nenhum dado para exibir no grafico dos tipos de trabalho.')
